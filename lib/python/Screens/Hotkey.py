...
# abilitiamo additional_keys per maggiore compatibilità (se vuoi lo riportiamo a False)
...
# --- Cobra default hotkeys (set only if empty) ---
def _set_default_if_empty(keyname, value):
	try:
		cfg = eval("config.misc.hotkey." + keyname)
		if not cfg.value or cfg.value.strip() == "":
			cfg.value = value
			cfg.save()
			print("[Hotkey] default set: %s -> %s" % (keyname, value))
	except Exception as e:
		print("[Hotkey] error setting default for %s: %s" % (keyname, e))

_set_default_if_empty("green", "Module/Screens.PluginBrowser/PluginBrowser")
_set_default_if_empty("red", "Infobar/servicelist/recallPrevService")
_set_default_if_empty("channelup", "Infobar/openFavouritesList")
# --- End Cobra defaults ---

# debug helper: stampa sui log i valori correnti (utile per troubleshooting)
def _debug_print_hotkeys():
	try:
		print("[Hotkey] green:", config.misc.hotkey.green.value)
		print("[Hotkey] red:", config.misc.hotkey.red.value)
		print("[Hotkey] channelup:", config.misc.hotkey.channelup.value)
		print("[Hotkey] additional_keys:", config.misc.hotkey.additional_keys.value)
	except Exception as e:
		print("[Hotkey] debug error:", e)

_debug_print_hotkeys()
