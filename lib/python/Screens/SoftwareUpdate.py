...
class UpdatePlugin(Screen, ProtectedScreen):
	...
	def checkTraficLight(self):
		...
		if status is None:
			message = _("You can't update the software because %s can not be reached.") % ("https://forums.cobraliberosat.net")
			abort = True
		else:
				...
				try:
					...
					if isinstance(message, dict):
						...
						if lang in message:
							...
							message = _("The current image might not be stable.\nFor more information see %s.") % ("https://forums.cobraliberosat.net")
			...
			# or display a generic warning if fetching failed
				...
				message = _("The status of the current image could not be checked because %s can not be reached.") % ("https://cobraliberosat.net")
